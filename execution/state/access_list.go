...
type accessList struct {
	...
	// Memo of the last resolved (address -> slot set); nil lastSlots means no
	// memo, so address-only entries never populate it. A call frame runs every
	// SLOAD/SSTORE against one address, so consecutive storage ops reuse the
	// memo instead of re-reading addresses. Must be dropped whenever a slot map
	// can be handed to a different address.
